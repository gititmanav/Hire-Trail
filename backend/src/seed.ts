...
async function seed(): Promise<void> {
  try {
    ...
    for (let i = 0; i < 650; i++) {
      ...
      const stageHistory: { stage: Stage; date: Date }[] = [
    ...
    // Create deadlines
    const appIds = (
      await Application.find({ userId }).select("_id").lean()
    ).map((a) => a._id);
    ...
    const total =
      1 + resumes.length + appDocs.length + contactDocs.length + deadlineDocs.length;
