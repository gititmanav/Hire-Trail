...
router.post("/", validate(emailTemplateSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
/** PUT /:id */
router.put("/:id", validate(updateEmailTemplateSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const admin = getUser(req);
