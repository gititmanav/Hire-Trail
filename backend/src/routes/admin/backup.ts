...
router.post("/export", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [users, applications, resumes, contacts, deadlines, auditLogs, announcements, settings, invites, templates] =
      await Promise.all([
...
router.post("/user-export/:userId", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const admin = getUser(req);
