...
router.get("/deadlines", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const filter: Record<string, unknown> = {};
    const now = new Date();
    if (status === "upcoming") { filter.completed = false; filter.dueDate = { $gte: now }; }
    else if (status === "overdue") { filter.completed = false; filter.dueDate = { $lt: now }; }
...
router.get("/resumes", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { page, limit, skip } = getPagination(req.query as Record<string, unknown>);
    const search = (req.query.search as string) || "";
