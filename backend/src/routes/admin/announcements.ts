...
router.post("/", validate(announcementSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.put("/:id", validate(updateAnnouncementSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const admin = getUser(req);
    ...
    if (req.body.startDate) announcement.startDate = new Date(req.body.startDate);
    if (req.body.endDate) announcement.endDate = new Date(req.body.endDate);
