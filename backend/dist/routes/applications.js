...
router.post("/", validate(createApplicationSchema), async (req, res, next) => {
    try {
        ...
        if (!companyId && req.body.company) {
            ...
            const company = await Company.findOneAndUpdate({ name: req.body.company.trim() }, {
...
router.post("/bulk", async (req, res, next) => {
    try {
        ...
        if (!Array.isArray(applications) || applications.length === 0)
            throw new ValidationError("applications must be a non-empty array");
        if (applications.length > 500)
            throw new ValidationError("Maximum 500 applications per import");
        const VALID_STAGES = ["Applied", "OA", "Interview", "Offer", "Rejected"];
        const docs = applications.map((app) => {
            if (!app.company || !app.role)
                throw new ValidationError("Missing company or role");
...
// PUT update
router.put("/:id", validate(updateApplicationSchema), async (req, res, next) => {
    try {
        const user = getUser(req);
        ...
        if (data.company !== undefined) {
            ...
            const company = await Company.findOneAndUpdate({ name: data.company.trim() }, {
        ...
        if (data.jobUrl !== undefined)
            existing.jobUrl = data.jobUrl;
        if (data.notes !== undefined)
            existing.notes = data.notes;
        if (data.resumeId !== undefined)
            existing.resumeId = data.resumeId;
        if (data.companyId !== undefined)
            existing.companyId = data.companyId;
        if (data.contactId !== undefined)
            existing.contactId = data.contactId;
        if (data.outreachStatus !== undefined)
            existing.outreachStatus = data.outreachStatus;
        if (data.archived !== undefined)
            existing.archived = data.archived;
        if (data.archivedAt !== undefined)
            existing.archivedAt = data.archivedAt ? new Date(data.archivedAt) : null;
        if (data.archivedReason !== undefined)
            existing.archivedReason = data.archivedReason;
        await existing.save();
        res.json(existing);
    }
    catch (err) {
...
router.put("/:id/archive", async (req, res, next) => {
    try {
        ...
        const app = await Application.findOneAndUpdate({ _id: req.params.id, userId: user._id }, { $set: { archived: true, archivedAt: new Date(), archivedReason: reason } }, { new: true });
        if (!app)
            throw new NotFoundError("Application");
        res.json(app);
    }
...
// PUT unarchive
router.put("/:id/unarchive", async (req, res, next) => {
    try {
        const user = getUser(req);
        const app = await Application.findOneAndUpdate({ _id: req.params.id, userId: user._id }, { $set: { archived: false, archivedAt: null, archivedReason: null } }, { new: true });
        if (!app)
            throw new NotFoundError("Application");
        res.json(app);
    }
