...
const envSchema = z
    ...
    if (localhostLike.test(data.CLIENT_URL)) {
        ctx.addIssue({
            ...
            message: "CLIENT_URL must be your public site URL in production (e.g. https://your-app.vercel.app), not localhost.",
            path: ["CLIENT_URL"],
        });
    }
    ...
    if (googleEnabled && localhostLike.test(data.GOOGLE_CALLBACK_URL)) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "GOOGLE_CALLBACK_URL must be your public API callback in production (e.g. https://your-app.vercel.app/api/auth/google/callback), not localhost.",
