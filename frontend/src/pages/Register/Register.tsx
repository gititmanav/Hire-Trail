      ...
      <div className="auth-left-panel hidden lg:flex flex-col justify-center items-center flex-1 relative overflow-hidden px-14 py-16">
        ...
        <div className="w-full max-w-[900px] relative z-10">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3" style={{ animation: "fadeSlideUp 0.55s ease-out 0.35s both" }}>
            <span className="auth-stat-chip text-xs text-foreground rounded-full px-3 py-1.5">Used by students and early-career professionals</span>
            <span className="auth-stat-chip text-xs text-foreground rounded-full px-3 py-1.5">Command center for applications</span>
          </div>

          <div className="mt-8" style={{ animation: "fadeSlideUp 0.55s ease-out 0.45s both" }}>
            <a
              href="https://github.com/gititmanav/Hire-Trail"
              target="_blank"
              rel="noopener noreferrer"
              className="auth-github-chip inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-medium text-foreground hover:text-primary transition-colors"
            >
      ...
      <div className="auth-right-panel flex-1 flex flex-col items-center justify-center px-6 py-12">
        <div className="auth-card w-full max-w-[430px] rounded-2xl p-6 sm:p-8 text-center" style={{ animation: "fadeSlideUp 0.5s ease-out" }}>
          ...
          <button type="button" disabled={maintenanceMode} onClick={() => { window.location.href = getGoogleOAuthUrl(); }}
            className="auth-oauth-btn w-full flex items-center justify-center gap-2.5 px-4 py-2.5 border border-border rounded-md text-sm font-medium shadow-sm transition-colors disabled:opacity-50 disabled:pointer-events-none">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-sm font-medium text-foreground mb-1.5">First Name</label><input type="text" className="auth-input-dark" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First Name" required disabled={maintenanceMode} /></div>
              <div><label className="block text-sm font-medium text-foreground mb-1.5">Last Name</label><input type="text" className="auth-input-dark" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Last Name" required disabled={maintenanceMode} /></div>
            </div>
            <div><label className="block text-sm font-medium text-foreground mb-1.5">Email Address</label><input type="email" className="auth-input-dark" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required disabled={maintenanceMode} /></div>
            <div><label className="block text-sm font-medium text-foreground mb-1.5">Password</label><input type="password" className="auth-input-dark" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 6 characters" minLength={6} required disabled={maintenanceMode} /></div>
