...
const FEATURES = [
  ...
  "Kanban board for visual pipeline",
  "Resume tracking with PDF upload",
  "Built-in job search powered by JSearch",
  "Custom analytics dashboard with widgets",
];
    ...
    <div className="flex min-h-screen">
      <div className="hidden lg:flex flex-col justify-center items-center flex-1 bg-[#f0f7fb] relative overflow-hidden px-12 py-16">
        ...
        <div className="max-w-md relative z-10">
          ...
          <h1 className="text-[32px] font-extrabold text-gray-900 leading-tight mb-2" style={{ animation: "fadeSlideUp 0.6s ease-out" }}>
            Your job search,
          </h1>
          <h1 className="text-[32px] font-extrabold text-gray-900 leading-tight mb-2" style={{ animation: "fadeSlideUp 0.6s ease-out 0.1s both" }}>
            organized and on track.
          </h1>
          <p className="text-[15px] text-gray-500 leading-relaxed mt-4 mb-10" style={{ animation: "fadeSlideUp 0.6s ease-out 0.2s both" }}>
          ...
          <div style={{ animation: "fadeSlideUp 0.5s ease-out 0.7s both" }}>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Trusted by students across universities</p>
            <div className="flex items-center gap-5 opacity-40 grayscale">
              {["Northeastern", "Purdue", "UCLA", "MIT", "Stanford"].map((u) => (
                <span key={u} className="text-[13px] font-bold text-gray-900 tracking-tight">{u}</span>
              ))}
            </div>
          </div>

          <div className="mt-12 pt-6 border-t border-gray-200" style={{ animation: "fadeSlideUp 0.5s ease-out 0.8s both" }}>
            <p className="text-xs text-gray-400">
              Built for students.{" "}
              <a href="https://github.com/gititmanav/Hire-Trail" target="_blank" rel="noopener noreferrer" className="text-accent hover:underline">
      ...
      <div className="flex-1 flex flex-col items-center justify-center bg-white dark:bg-gray-900 px-6 py-12">
        <div className="w-full max-w-[400px] text-center" style={{ animation: "fadeSlideUp 0.5s ease-out" }}>
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">Sign up for an account</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-7">Start tracking your job search today</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            ...
            <button type="submit" disabled={loading} className="btn-accent w-full justify-center !py-2.5 mt-2 disabled:opacity-50">{loading ? "Creating account..." : "Register"}</button>
          </form>
