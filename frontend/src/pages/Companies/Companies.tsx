...
import { companiesAPI, resumesAPI, contactsAPI, deadlinesAPI } from "../../utils/api.ts";
...
function AppSidebar({ app, resumes, contacts, deadlines, onClose }: {
  app: Application; resumes: Resume[]; contacts: Contact[]; deadlines: Deadline[]; onClose: () => void;
}) {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-[420px] bg-card shadow-2xl z-50 overflow-y-auto animate-in border-l border-border">
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1 block">Stage</label>
            <span className={`inline-block px-3 py-1 text-xs font-medium rounded-full ${badgeCls[app.stage]}`}>{app.stage}</span>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-sm text-secondary-foreground">{resume ? resume.name : "None"}</p>
          </div>
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1 block">Contact</label>
            {companyContacts.length > 0 ? companyContacts.slice(0, 3).map((c) => (
              <p key={c._id} className="text-sm text-secondary-foreground">{c.name} · {c.role}</p>
            )) : <p className="text-sm text-muted-foreground">None</p>}
          </div>
          <div>
            <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1 block">Deadlines</label>
            {appDeadlines.length > 0 ? appDeadlines.map((d) => (
              <div key={d._id} className="flex justify-between text-sm"><span className="text-secondary-foreground">{d.type}</span><span className="text-xs text-muted-foreground">{fmt(d.dueDate)}</span></div>
            )) : <p className="text-sm text-muted-foreground">None</p>}
          </div>
          {app.notes && <div><label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-1 block">Notes</label><p className="text-sm text-secondary-foreground whitespace-pre-wrap">{app.notes}</p></div>}
        </div>
      </div>
    </>
  );
    ...
    <div className="fade-up">
      ...
      {sidebarApp && (
        <AppSidebar
          ...
          contacts={contacts}
          deadlines={deadlines}
          onClose={() => { setSidebarApp(null); if (!sidebarCompany) setSidebarCompany(null); }}
        />
      )}
    </div>
  );
