...
import {
  X, Minus, LayoutGrid, BarChart3, ChevronsRight, TrendingUp, PieChart, ClipboardList,
  AlignLeft, Clock, Calendar, type LucideIcon,
} from "lucide-react";
...
const IC: Record<string, LucideIcon> = {
  stats: LayoutGrid,
  funnel: BarChart3,
  conversion: ChevronsRight,
  trend: TrendingUp,
  pie: PieChart,
  "resume-perf": ClipboardList,
  "recent-apps": AlignLeft,
  deadlines: Clock,
...
export default function WidgetPicker({ visible, onToggle, onReset, onClose }: Props) {
  ...
  return (
    <div ref={exitRef} className="fixed inset-0 bg-scrim/45 flex items-center justify-center z-50 modal-overlay-in" onClick={onClose}>
      <div data-modal-panel className="bg-card rounded-xl p-6 w-full max-w-[480px] shadow-2xl animate-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-between mt-5 pt-4 border-t border-border"><button onClick={onReset} className="text-sm text-muted-foreground hover:text-danger">Reset to defaults</button><button onClick={onClose} className="px-4 py-2 text-sm font-medium text-primary-foreground bg-primary hover:bg-primary/90 rounded-lg">Done</button></div>
      </div>
    </div>
  );
}
