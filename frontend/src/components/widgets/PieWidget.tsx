...
const COLORS = ["#378ADD", "#EF9F27", "#7F77DD", "#1D9E75", "#E24B4A"];
...
export default function PieWidget({ data }: Props) {
  ...
  const options = { cutout: "60%", plugins: { tooltip: { backgroundColor: "rgba(0,0,0,0.8)", padding: 10, cornerRadius: 8, callbacks: { label: (ctx: any) => { const pct = total > 0 ? Math.round((ctx.parsed / total) * 100) : 0; return ` ${ctx.label}: ${ctx.parsed} (${pct}%)`; } } }, legend: { display: true, position: "bottom" as const, labels: { boxWidth: 10, boxHeight: 10, borderRadius: 2, useBorderRadius: true, padding: 12, font: { size: 11 }, color: "#9ca3af" } } } };
  return <div className="h-full w-full flex items-center justify-center p-2"><Doughnut data={chartData} options={options} /></div>;
}
