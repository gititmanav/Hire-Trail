...
export default function ConversionWidget({ data }: Props) {
  ...
  return (<div className="space-y-4 h-full flex flex-col justify-center">{conv.map((c) => (<div key={c.to}><div className="flex items-center justify-between mb-1.5"><span className="text-[13px] font-medium text-gray-500 dark:text-gray-400">{c.from} &rarr; {c.to}</span><span className="text-xs text-gray-400">{c.count}/{c.prev}</span></div><div className="h-2.5 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden"><div className="h-full bg-accent rounded-full transition-all duration-500" style={{ width: `${Math.min(c.rate, 100)}%` }} /></div><div className="text-right mt-1"><span className="text-lg font-bold text-gray-900 dark:text-white">{c.rate}%</span></div></div>))}</div>);
}
